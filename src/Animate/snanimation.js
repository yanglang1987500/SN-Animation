...
SNAnimation.Layer.prototype = {
  ...
  start: function () {
    for (var i = 0; i < this.displayObjectArray.length; i++) {
      const timer = this.displayObjectArray[i].getTimer();
      if(timer) {
        timer.start();
      }
    }
...
SNAnimation.Renderer = {
  ...
  clear: function () {
    ...
    this.layers = [];
    this._clearCanvas();
    PubSub.unsubscribeLike('__SNANI_IMAGELOAD_');
    PubSub.unsubscribeLike('onorientationchange');
    PubSub.unsubscribeLike('__SNANI_rendernotify');
  },
  clearLayer: function(){
    for (var i = 0; i < this.layers.length; i++) {
      this.layers[i].stop();
